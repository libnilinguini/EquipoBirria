...
def inscribirse(curso_id: int):
    ...
    # 5b · Verificar código de acceso
    if curso.codigo_acceso:
        codigo_ingresado = request.form.get("codigo_acceso", "").strip()
        if codigo_ingresado != curso.codigo_acceso:
            flash("Código de acceso incorrecto. Solicítalo al profesor.", "danger")
            return redirect(url_for("inscripciones.cursos_disponibles"))
